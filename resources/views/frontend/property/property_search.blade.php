...
<meta property="og:image" content="{{ asset('frontend/images/banner.webp') }}" />
...
<meta name="twitter:image" content="{{ asset('frontend/images/banner.webp') }}" />
@endsection
@section('main')
...
function fetchSearchResults() {
    ...
    // 🔁 Old logic (still works if hidden fields exist)
    formData.set('lat', document.getElementById('form-lat').value);
    formData.set('lon', document.getElementById('form-lon').value);

    // 🟢 New: Try to get dynamic map center and radius if Google Map is available
    // Use window.map explicitly here to avoid scope issues
    if (typeof window.map !== "undefined" && window.map.getBounds && window.map.getCenter) {
        ...
        const radiusKm = haversineDistance(centerLat, centerLon, ne.lat, ne.lng);

        formData.set('lat', centerLat);
        formData.set('lon', centerLon);
        formData.set('radius', Math.round(radiusKm));
    }
    ...
    .then(data => {
        ...
        initSwipers(); // Re-init sliders/swipers if needed
    })
    .catch(err => {
