...
(function ($) {
        ...
        $(function () {
            function mmenuInit() {
                ...
                if (wi <= "1024") {
                    ...
                    // 🔻 Append Login Link Only for Mobile Menu

                    if (window.isAuthenticated) {
                        $(".mmenu-init ul:first").append(`
                        <li><a href="/dashboard"><i class="sl sl-icon-wrench"></i> การจัดการ</a></li>
                        <li><a href="/user/add/listing"><i class="sl sl-icon-plus"></i> ลงประกาศ</a></li>
                        <li><a href="/user/logout"><i class="sl sl-icon-power"></i> ออกระบบ</a></li>
                    `);
                    } else {
                        $(".mmenu-init ul:first").append(`
                        <li><a href="/login">เข้าสู่ระบบ</a></li>
                        <li><a href="/user/add/listing">ลงประกาศ</a></li>
                    `);
                    }
                            ...
                            navbars: [
                                {
                                    position: "bottom",
                                    content: [
                                        // "<a class='login-nav' href='#/'> เข้าระบบ</a>",
                                        // "<a class='login-nav' href='#/'>ลงประกาศ</a>",
                                    ],
                                },
                            ],
                        },
                    ...
                    $(".mmenu-trigger").click(function () {
                        // 👇 Patch: Blur any focused element inside the original nav
                        const $navigation = $("#navigation");
                        if (
                            $navigation.length &&
                            $navigation[0].contains(document.activeElement)
                        ) {
	...
	/*----------------------------------------------------*/
        $("#header")
            ...
            .insertAfter("#header");
        $("#header.cloned #sign-in-dialog #terms-modal #policy-modal").remove();
        $("#navigation.style-2")
            .clone(true)
            .addClass("cloned unsticky")
	...
	/*----------------------------------------------------*/
        jQuery(document).ready(function ($) {
            $('a[href="#sign-in-dialog"]').magnificPopup({
                type: "inline",
            ...
            $(".parallax").each(function (i) {
                ...
                if (bottomWindow > top && topWindow < bottom) {
                    ...
                    var overflowH =
                        height < heightWindow
                            ? imgH - height
                            : imgH - heightWindow; // fix height on overflow
                    top = top - overflowH;
    ...
    /*----------------------------------------------------*/
        ...
        var config = {
            ".chosen-select": { disable_search_threshold: 15, width: "100%" },
    ...
    /*----------------------------------------------------*/
        ...
        $(window).on("load resize", function (e) {
            var carouselListItems = $(
                ".listing-slider-small .slick-track"
            ).children().length;
	...
	/*----------------------------------------------------*/
        ...
        $trigger.on("click", function (e) {
            ...
            if ($(this).next().is(":hidden")) {
                ...
                $triggerloc
                    .find("span")
                    .removeClass("ui-accordion-icon-active");
            ...
            daySlots.find(".add-slot-btn").on("click", function () {
                ...
                // Validation Error
                    ...
                    setTimeout(function () {
                        daySlots
                            .find(".add-slot")
                            .removeClass("add-slot-shake-error");
                    }, 600);
                }
            });
...
/*  Confirm Wishlist Delete
/*--------------------------*/
function deleteFromWishList(itemId, deleteUrl, csrfToken) {
    Swal.fire({
        title: "คุณแน่ใจหรือไม่?",
        text: "คุณต้องการลบประกาศนี้ออกจากรายการโปรด",
        icon: "warning",
        ...
        customClass: {
            popup: "swal-wide", // 👈 You can define this class below
        },
    }).then((result) => {
            ...
            $.ajax({
                ...
                success: function (response) {
                    if (response.success) {
                        Swal.fire({
                            title: "ลบแล้ว!",
                            text: "ประกาศถูกลบออกจากรายการโปรด",
                            icon: "success",
                            customClass: "swal-wide",
                        });
                        $(`a.delete-wishlist[data-id='${itemId}']`)
                            .closest("li")
                            .remove();
                    } else {
                        Swal.fire({
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    form.addEventListener("submit", function (e) {
        ...
        // Name
        if (name.value.trim() === "") {
            document.getElementById("error-name1").innerText = "กรุณากรอกชื่อ";
        ...
        if (email.value.trim() === "") {
            document.getElementById("error-email2").innerText =
                "กรุณากรอกอีเมล์";
